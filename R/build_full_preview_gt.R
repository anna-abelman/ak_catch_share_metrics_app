# --- Helper 1: Build the Modal UI ---
show_preview_modal <- function(ns, table_id, download_id, title = "Full Dataset Preview") {
  showModal(
    modalDialog(
      title = title,
      size = "xl", 
      ...
      # Increased negative margin to override gt's default spacing
      div(
        style = "margin-top: -30px; overflow-x: auto; width: 100%;",
        gt_output(ns(table_id))
      ),
      
      footer = tagList(
        downloadButton(ns(download_id), "Download CSV", class = "btn-primary"),
        modalButton("Close")
      )
    )
  )
...
# --- Helper 2: Build the Scrolling GT Table (Modal) ---
build_preview_gt <- function(raw_data, header_bg = "#1E293B") {
  
  wide_data <- raw_data %>%
    pivot_wider(names_from = varname, values_from = Value)
  
  has_sector <- "sector" %in% names(wide_data)
  
  # SAFELY SORT STRINGS LIKE "2011/2012" BY GRABBING THE FIRST 4 CHARACTERS
  if (has_sector) {
    wide_data <- wide_data %>%
      mutate(sort_col = suppressWarnings(as.numeric(substr(as.character(Season), 1, 4)))) %>%
      arrange(sector, !is.na(sort_col), sort_col) %>% 
      select(-sort_col) 
  } else {
    wide_data <- wide_data %>%
      mutate(sort_col = suppressWarnings(as.numeric(substr(as.character(Season), 1, 4)))) %>%
      arrange(!is.na(sort_col), sort_col) %>%
      select(-sort_col) 
  }
  
  if (has_sector) {
    gt_tbl <- wide_data %>% gt(rowname_col = "Season", groupname_col = "sector")
  } else {
    gt_tbl <- wide_data %>% gt(rowname_col = "Season")
  }
  ...
  gt_tbl %>% 
    ...
    # Apply nowrap to all columns EXCEPT 'ACL exceeded species'
    tab_style(
      style = cell_text(whitespace = "nowrap"),
      locations = cells_body(
        columns = !any_of("ACL exceeded species") 
      )
    ) %>%
    tab_footnote("'-' indicates data not available for metric. Confidential data is suppressed and specified by 'Conf.'.")
}

# --- Helper 3: Build the Main Filtered Metrics Table ---
build_metrics_gt <- function(cleaned_data, selected_vars, header_bg = "#1E293B") {
  ...
  raw_filtered <- cleaned_data %>% 
    ...
    mutate(
      varname = factor(varname, levels = selected_vars), 
      Season = as.character(Season),
      Value_num = suppressWarnings(as.numeric(str_remove_all(Value, "[\\$, ]"))),
      Value_num = na_if(Value_num, 0) 
    ) 
  
  # SAFELY HANDLE "2011/2012" SEASONS FOR CHRONOLOGICAL SORTING
  all_seasons <- unique(raw_filtered$Season)
  non_baseline_seasons <- all_seasons[all_seasons != "BASELINE"]
  
  # Sort by grabbing just the first 4 digits of the season string
  sorted_seasons <- non_baseline_seasons[order(suppressWarnings(as.numeric(substr(non_baseline_seasons, 1, 4))))]
  correct_season_order <- c("BASELINE", as.character(sorted_seasons))
  ...
  last_5_years <- tail(as.character(sorted_seasons), 5)
  ...
  final_table <- gt_tbl %>%
    ...
    tab_style(
      style = cell_text(whitespace = "nowrap"),
      locations = cells_body(
        rows = varname != "ACL exceeded species"
      )
    ) %>%
